...
public class MainApp {
   public static void main(String[] args) throws SQLException {
      ...
      for (User user : users) {
         String model = user.getCar().getModel();
         int series = user.getCar().getSeries();
         System.out.println("Владелец машины "+model+", series"+series+" - это "+userService.getTargetUser(model, series));
      }
      context.close();
   }
}
